...
const parseProjectInfo = (data, projectName) => {
  ...
  let projLanguage = '';
  if (datastream?.attrs?.language) {
    projLanguage = datastream.attrs.language;
  }
  let spacer = Math.max(
    projLabel.length,
...
├────────────────────────────┼────────────────${'─'.repeat(strecher)}┤
│ Auditor refresh timer:     │ ${auditorRefresh}${' '.repeat(spacer - auditorRefresh.length)}│${projLanguage.length != 0? `
├────────────────────────────┼────────────────${'─'.repeat(strecher)}┤
